...
class OrganisationPicker extends StatefulWidget {
  ...
  final void Function(Organisation? l) onSelect;

  final Organisation? initialValue;

  final String? label;

  Future<Result<List<Organisation>>> getEntities(
    String filterName,
    LoadProps? loadProps,
  ) {
    return repository
        ...
        .then((v) {
          switch (v) {
            case Ok<ResultList<Organisation>>():
              return Result.ok(v.value.entities);
            case Error<ResultList<Organisation>>():
  ...
  @override
  State<OrganisationPicker> createState() => OrganisationPickerState();
}

class OrganisationPickerState extends State<OrganisationPicker> {
  Organisation? currentValue;

  final int take = 25;

  late TextEditingController nameController;
  late PagingController<int, Organisation> pagingController;
  ...
  Future<List<Organisation>> getData(String filterName, LoadProps? loadProps) {
    return widget.getEntities(filterName, loadProps).then((result) {
      switch (result) {
        case Ok<List<Organisation>>():
          return result.value;
        case Error<List<Organisation>>():
          return Future.value([]);
      }
    });
  }

  void addOrganisation(BuildContext context) async {
    ...
    var p = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => PopScope(
          ...
          child: OrganisationDetails(viewmodel: vm),
        ),
      ),
    );
    if (p != null && p is Organisation) {
      select(p);
      if (context.mounted) {
  ...
  String entityAsString(Organisation? entity) {
    if (entity == null) {
      return "";
    }
  ...
  void openBottomSheet(BuildContext context) async {
    final paging = PagingListener(
      ...
      builder: (context, state, fetchNextPage) {
        return PagedSliverList<int, Organisation>(
          state: state,
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  onPressed: () => addOrganisation(context),
                  icon: Icon(Icons.add),
                ),
              ],
            ),
